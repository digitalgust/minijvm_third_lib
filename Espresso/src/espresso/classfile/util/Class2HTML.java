    ...
    implements Constants
{
   ...
   */
        ...
        throws IOException
    {
        ...
        if (index > -1) class_package =  class_name.substring(0, index); else class_package =  ""; // default package
       ...
       */
            for (int i =  0; i < argv.length; i++) {
                if (argv[i].charAt(0) == '-') { // command line switch
                    ...
                    } else if (argv[i].equals("-zip")) zip_file =  argv[++i]; else System.out.println("Unknown option " + argv[i]);
                } else// add file name to list */
                 file_name[files++] =  argv[i];
            }

            if (files == 0) System.err.println("Class2HTML: No input files specified."); else { // Loop through files ...
                for (int i =  0; i < files; i++) {
                    System.out.print("Processing " + file_name[i] + "...");
                    if (zip_file == null) parser =  new ClassParser(file_name[i]); // Create parser object from file
                    else parser =  new ClassParser(zip_file, file_name[i]); // Create parser object from zip file
   ...
   */
    ...
    static final String referenceType (String type) {
        ...
        // test for basic type
        if (type.equals("int") || type.equals("short") || type.equals("boolean") || type.equals("void") || type.equals("char") || type.equals("byte") || type.equals("long") || type.equals("double") || type.equals("float")) return  "<FONT COLOR=\"#00FF00\">" + type + "</FONT>"; else return  "<A HREF=\"" + type + ".html\" TARGET=_top>" + short_type + "</A>";
    }
