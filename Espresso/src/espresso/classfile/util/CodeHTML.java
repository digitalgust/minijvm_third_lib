...
final class CodeHTML
    implements espresso.classfile.Constants
{
   ...
   */
    private final String codeToHTML (ByteSequence bytes, int method_number)
        throws IOException
    {
        short opcode =  Utility.byteToShort(bytes.getByte());
        StringBuffer buf;
     ...
     */
        if ((opcode == TABLESWITCH) || (opcode == LOOKUPSWITCH)) {
            ...
            for (int i =  0; i < no_pad_bytes; i++) bytes.getByte();
            // Both cases have a field default_offset in common
            default_offset =  Utility.bytesToInt(bytes.getByte(), bytes.getByte(), bytes.getByte(), bytes.getByte());
        }

        switch (opcode) {
            case TABLESWITCH:
                low =  Utility.bytesToInt(bytes.getByte(), bytes.getByte(), bytes.getByte(), bytes.getByte());
                high =  Utility.bytesToInt(bytes.getByte(), bytes.getByte(), bytes.getByte(), bytes.getByte());
                ...
                for (int i =  0; i < jump_table.length; i++) {
                    jump_table[i] =  offset + Utility.bytesToInt(bytes.getByte(), bytes.getByte(), bytes.getByte(), bytes.getByte());
       ...
       */
            ...
            case LOOKUPSWITCH:
                int npairs =  Utility.bytesToInt(bytes.getByte(), bytes.getByte(), bytes.getByte(), bytes.getByte());
                ...
                for (int i =  0; i < npairs; i++) {
                    int match =  Utility.bytesToInt(bytes.getByte(), bytes.getByte(), bytes.getByte(), bytes.getByte());
                    jump_table[i] =  offset + Utility.bytesToInt(bytes.getByte(), bytes.getByte(), bytes.getByte(), bytes.getByte());
       ...
       */
            ...
            case IF_ICMPNE:
            case JSR:
                index =  (int)(bytes.getIndex() + Utility.bytesToShort(bytes.getByte(), bytes.getByte()) - 1);
       ...
       */
            ...
            case GOTO_W:
            case JSR_W:
                int windex =  bytes.getIndex() + Utility.bytesToInt(bytes.getByte(), bytes.getByte(), bytes.getByte(), bytes.getByte()) - 1;
       ...
       */
            ...
            case RET:
                if (wide) {
                    vindex =  Utility.bytesToShort(bytes.getByte(), bytes.getByte());
                    wide =  false; // Clear flag
                } else vindex =  Utility.byteToShort(bytes.getByte());
       ...
       */
            ...
            case NEWARRAY:
                buf.append("<FONT COLOR=\"#00FF00\">" + TYPE_NAMES[bytes.getByte()] + "</FONT>");
                break;
       ...
       */
            ...
            case PUTFIELD:
            case PUTSTATIC:
                index =  Utility.bytesToShort(bytes.getByte(), bytes.getByte());
       ...
       */
            ...
            case INVOKEINTERFACE:
                int m_index =  Utility.bytesToShort(bytes.getByte(), bytes.getByte());
                String str;

                if (opcode == INVOKEINTERFACE) { // Special treatment needed
                    int nargs =  Utility.byteToShort(bytes.getByte()); // Redundant
                    int reserved =  Utility.byteToShort(bytes.getByte()); // Reserved
       ...
       */
            ...
            case LDC:
                index =  Utility.byteToShort(bytes.getByte());
                buf.append("<A HREF=\"" + class_name + "_cp.html#cp" + index + "\" TARGET=\"ConstantPool\">" + Class2HTML.toHTML(constant_pool.constantToString(index, constant_pool.getConstant(index).getTag())) + "</a>");
                break;
                /* Array of references.
       */
       ...
       */
            ...
            case IINC:
                if (wide) {
                    vindex =  Utility.bytesToShort(bytes.getByte(), bytes.getByte());
                    constant =  Utility.bytesToShort(bytes.getByte(), bytes.getByte());
                    wide =  false;
                } else {
                    vindex =  Utility.byteToShort(bytes.getByte());
                    constant =  bytes.getByte();
                }
                ...
                if (NO_OF_OPERANDS[opcode] > 0) {
                    for (int i =  0; i < TYPE_OF_OPERANDS[opcode].length; i++) {
                        switch (TYPE_OF_OPERANDS[opcode][i]) {
                            case T_BYTE:
                                buf.append(Utility.byteToShort(bytes.getByte()));
                                break;

                            case T_SHORT: // Either branch or index
                                buf.append(Utility.bytesToShort(bytes.getByte(), bytes.getByte()));
                                break;

                            case T_INT:
                                buf.append(Utility.bytesToInt(bytes.getByte(), bytes.getByte(), bytes.getByte(), bytes.getByte()));
                                break;
   ...
   */
     ...
     */
        ...
        for (int i =  0; bytes.available() > 0; i++) {
            opcode =  Utility.byteToShort(bytes.readByte());

            switch (opcode) {
                case TABLESWITCH:
                case LOOKUPSWITCH:
                    bytes.getByte(); // Skip already read byte
                    int remainder =  bytes.getIndex()%4;
                    int no_pad_bytes =  (remainder == 0)? 0 : 4 - remainder;
                    int default_offset, offset;
                    for (int j =  0; j < no_pad_bytes; j++) bytes.getByte();
                    // Both cases have a field default_offset in common
                    default_offset =  Utility.bytesToInt(bytes.getByte(), bytes.getByte(), bytes.getByte(), bytes.getByte());

                    if (opcode == TABLESWITCH) {
                        int low =  Utility.bytesToInt(bytes.getByte(), bytes.getByte(), bytes.getByte(), bytes.getByte());
                        int high =  Utility.bytesToInt(bytes.getByte(), bytes.getByte(), bytes.getByte(), bytes.getByte());
                        offset =  bytes.getIndex() - 12 - no_pad_bytes - 1;
                        default_offset +=  offset;
                        goto_set.set(default_offset);

                        for (int j =  0; j < (high - low + 1); j++) {
                            index =  offset + Utility.bytesToInt(bytes.getByte(), bytes.getByte(), bytes.getByte(), bytes.getByte());
                            goto_set.set(index);
                        }
                    } else { // LOOKUPSWITCH
                        int npairs =  Utility.bytesToInt(bytes.getByte(), bytes.getByte(), bytes.getByte(), bytes.getByte());
                ...
                case JSR:
                    bytes.getByte(); // Skip already read byte
                    index =  bytes.getIndex() + Utility.bytesToShort(bytes.getByte(), bytes.getByte()) - 1;
                    goto_set.set(index);
                    break;
                ...
                case JSR_W:
                    bytes.getByte(); // Skip already read byte
                    index =  bytes.getIndex() + Utility.bytesToInt(bytes.getByte(), bytes.getByte(), bytes.getByte(), bytes.getByte()) - 1;
                    goto_set.set(index);
                    break;
   ...
   */
        ...
        throws IOException
    {
        ...
        if (attributes.length > 0) {
            ...
            for (int i =  0; i < attributes.length; i++) {
                ...
                if (tag != ATTR_UNKNOWN) file.print("<LI><A HREF=\"" + class_name + "_attributes.html#method" + method_number + "@" + i + "\" TARGET=Attributes>" + ATTRIBUTE_NAMES[tag] + "</A></LI>\n"); else file.print("<LI>" + attributes[i] + "</LI>");
	 ...
	 */
                ...
                anchor2 =  "<A NAME=code" + method_number + "@" + code.length + ">" + offset + "</A>"; else anchor2 =  "" + offset;
